...
def perspective_transform(image, corners):
    def order_corner_points(corners):
        ...
        top_r, top_l, bottom_l, bottom_r = corners[0], corners[1] if len(corners) > 1 else corners[-1], corners[2]  if len(corners) > 2 else corners[-1], corners[3] if len(corners) > 3 else corners[-1]
...
margin = len(canny) // 5
canny[0:margin, :] = [[0] * canny.shape[1]] * margin
canny[-margin:, :] = [[0] * canny.shape[1]] * margin
canny[:, 0:margin] = [[0] * margin] * canny.shape[0]
canny[:, -margin:] = [[0] * margin] * canny.shape[0]

corners = cv2.goodFeaturesToTrack(canny, 4, 0.7, 2)
...
c_sorted = sorted(corners, key=lambda x: (x[0][1], x[0][0]))
c_sorted_1 = sorted(corners, key=lambda x: (x[0][0], x[0][1]))

corner_points = np.array([c_sorted[0][0], c_sorted[0][-1], c_sorted_1[0][0], c_sorted_1[0][-1]])
